...
public void validarIdentificacionUnica(String identificacion) {
    ...
    public static void validarTipoActividad(String tipo) {
        ...
        if (!(tipoUpper.equals("RECREATIVA") || tipoUpper.equals("EDUCATIVA") || tipoUpper.equals("LABORAL"))) {
            JOptionPane.showMessageDialog(null, "Tipo de actividad no válido. Solo se permite: Recreativa, Educativa o Laboral");
        }
    }

    public static void validarDiaActividad(Object dia) {
        if (dia == null || dia.toString().equals("<Seleccionar>")) {
            JOptionPane.showMessageDialog(null, "Debe seleccionar un día válido para la actividad", "Error", JOptionPane.ERROR_MESSAGE);
            return;
        }
        ...
        if (!diasValidos.contains(valor)) {
            JOptionPane.showMessageDialog(null, "Día seleccionado no es válido", "Error", JOptionPane.ERROR_MESSAGE);
        }
    }
    ...
    public static void validarCupoMaximo(String cupoStr) {
        ...
        try {
            ...
            if (cupo <= 0 || cupo > 50) {
                throw new IllegalArgumentException("El cupo máximo debe estar entre 1 y 50 personas");
            }
        } catch (NumberFormatException e) {
